...
class CircularSeekBar : View {
    ...
    var mEndAngle = 0f
        set(angle) {
            field = if (mStartAngle % 360f == angle % 360f) {
    ...
    var mPointerAlpha = DEFAULT_POINTER_ALPHA
         ...
         * @param alpha the alpha of the pointer (0..255)
         */
        set(alpha) {
            if (alpha in 0..255) {
                field = alpha
     ...
     */
    private lateinit var mCirclePointerPath: Path
    ...
    private fun initAttributes(attrArray: TypedArray) {
        ...
        isNegativeEnabled = attrArray.getBoolean(R.styleable.cs_CircularSeekBar_cs_negative_enabled, DEFAULT_NEGATIVE_ENABLED)
    ...
    private fun initPaints() {
        mCirclePaint = Paint().apply {
            ...
            style = STROKE
            strokeJoin = ROUND
            strokeCap = mCircleStyle
        }
        mCircleFillPaint = Paint().apply {
            isAntiAlias = true
            isDither = true
            color = mCircleFillColor
            style = FILL
        }
        mCircleProgressPaint = Paint().apply {
            isAntiAlias = true
            isDither = true
        ...
        if (!mDisableProgressGlow) {
            mCircleProgressGlowPaint = Paint().apply {
                set(mCircleProgressPaint)
                maskFilter = BlurMaskFilter(PROGRESS_GLOW_RADIUS_DP * DPTOPX_SCALE, NORMAL)
            }
        }
        mPointerPaint = Paint().apply {
            isAntiAlias = true
            isDither = true
            color = mPointerColor
            strokeWidth = mPointerStrokeWidth
            style = STROKE
            strokeJoin = ROUND
            strokeCap = mCircleStyle
        }
        mPointerHaloPaint = Paint().apply {
            set(mPointerPaint)
            color = mPointerHaloColor
            alpha = mPointerAlpha
            strokeWidth = mPointerStrokeWidth + mPointerHaloWidth * 2f
        }
        mPointerHaloBorderPaint = Paint().apply {
            set(mPointerPaint)
            strokeWidth = mPointerHaloBorderWidth
            style = STROKE
        }
    }
    ...
    private fun calculateProgressDegrees() {
        mProgressDegrees = if (mIsInNegativeHalf) mStartAngle - mPointerPosition else mPointerPosition - mStartAngle // Verified
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val ableToGoNegative = isNegativeEnabled && Math.abs(mTotalCircleDegrees - 360f) < SMALL_DEGREE_BIAS * 2
...
//            }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val isHardwareAccelerated = VERSION.SDK_INT >= VERSION_CODES.HONEYCOMB &&
        ...
        val padding = Math.max(mCircleStrokeWidth / 2f,
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        additionalRadius = if (mPointerStrokeWidth < minimumTouchTarget / 2) { // If the pointer radius is less than the minimumTouchTarget, use the minimumTouchTarget
            minimumTouchTarget / 2
        } else {
        ...
        when (event.action) {
            MotionEvent.ACTION_DOWN -> {
                ...
                val pointerRadiusDegrees = (mPointerStrokeWidth * 180 / (Math.PI * Math.max(mCircleHeight, mCircleWidth))).toFloat()
    ...
    private fun init(attrs: AttributeSet?, defStyleAttr: Int, defStyleRes: Int) {
        initPaints()
        initPaths()
        val attrArray = context.obtainStyledAttributes(attrs, R.styleable.cs_CircularSeekBar, defStyleAttr, defStyleRes)
